...
router.post('/', async (req,res) => {
    ...
    try {
        ...
        // if (Checkuser.length >2 ) { return res.status(200).json({ status: false,message: 'Wishlist Already Exists' }) }
            ...
            user_id,scheme,duration,installment,amount
         })
        
        await wishlist.save()
        res.status(200).send({ status: "true",message: 'SchemeList Saved',data:wishlist})
    } catch (err) {
...
router.put('/:id',authenticate, async (req,res) => {
    SchemeList.findByIdAndUpdate(req.params.id, req.body, (err, user) => {
        ...
        res.status(200).send({ status: "true",message: 'SchemeList Updated Success',data:user})
    });
})
...
router.delete('/:id',authenticate, async (req, res) => {
    SchemeList.findByIdAndRemove(req.params.id, req.body, (err, user) => {
        ...
        res.status(200).send({ status: "true",message: 'SchemeList Deleted Success',data:user})
    });
})
...
router.get("/",authenticate,async (req, res) => {
        ...
        try {
            ...
            const results = await SchemeList.find({user_id:user_id}).populate('scheme').exec();
            ...
            res.status(200).send({ status: "true",message: 'SchemeList List Loading Success', data:results})
        } catch (err) {
    ...
    try {
     const results = await SchemeList.find({user_id:user_id}).populate('scheme').exec();
        ...
        for (var i in results) {
            ...
            const PAYDETAIL =  await Payment.find({user_id:ObjectId(user_id),scheme:ObjectId(scheme)}).exec();
           ...
           console.log(tdate)
            const PAY_BTN_HIDE =  await Payment.find({scheme:(scheme),createdAt: { '$gte': fdate, '$lte':tdate } });
            // console.log('sfs')
          
            schme_duration = results[0].duration
            schme_startdate = results[0].createdAt
            
            schme_enddate=moment(schme_startdate).add(schme_duration, 'months').format('YYYY-MM-DD hh:mm:ss');
            // console.log('asdf')

            let userSchemeDetails = await Payment.find({user_id: user_id, scheme: scheme})

            let SchemeDetails = await Schemes.findById({_id: ObjectId(scheme)})

            let getInstallment = parseInt(SchemeDetails.installment) - parseInt(userSchemeDetails.length)
        ...
        res.status(200).send({ status: "true",message: 'User Info Success', data:Resultarray}) 
    } catch (err) {
        res.status(200).send({ status: "false",message: 'Error in Solving', data:err})
    }
});

// router.post('/payment-stat', async (req,res) => {

//     userId = req.body.user_id
//     schemeId = req.body.scheme_id

//     let userSchemeDetails = await UserScheme.find({user_id: userId, scheme: schemeId})
//     let SchemeDetails = await Schemes.findById({_id: ObjectId(schemeId)})
//     let getInstallment = parseInt(SchemeDetails.installment) - parseInt(userSchemeDetails.length)
...
//         totalInstallments: SchemeDetails.installment
//     }
//     let schemeDetails = await UserScheme.find({user_id: userId, scheme: schemeId}).distinct('scheme')

//     let  schemelistArr = []

//     let SchemeDetailsList = await Schemes.findById({_id: ObjectId(schemeId)})

//     schemelistArr.push({
//         schemeDetails: SchemeDetailsList,
//         installmentDetails: makeArr,
//         // paymentHistory: userSchemeDetails
//     })

    

//     return res.json({data:schemelistArr})
