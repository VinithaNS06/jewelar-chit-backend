...
router.post("/", async (req, res) => {
  ...
  try {
    const Checkcustomer = await Order.findOne({
      customer_id,
      product_id,
      transaction_id,
      total_amount,
    ...
    if (Checkcustomer) {
      return res
        .status(200)
        .json({ status: false, message: "Order Already Exists" });
    }
    ...
    res
      .status(200)
      .send({ status: "true", message: "Order Saved", data: category });
  } catch (err) {
...
router.get("/", async (req, res) => {
  try {
    ...
    const results = await Order.find({ status: 1 }).populate([
      "customer_id",
      "product_id",
    ]);
...
router.get("/:id", async (req, res) => {
  Order.find({ _id: req.params.id }, (err, docs) => {
    if (!err) {
      res.status(200).send({
        status: "true",
        message: "Order List Loading Success",
...
router.get("/customerorders/:id", async (req, res) => {
  ...
  Order.find({ customer_id: req.params.id }, (err, docs) => {
    if (!err) {
      res.status(200).send({
        status: "true",
        message: "Order List Loading Success",
...
router.get("/orderlist/totalorder", async (req, res) => {
  ...
  for (let i = 0; i < customersData.length; i++) {
    const customerOrders = await Order.find({
      customer_id: customersData[i].customer_id,
    }).populate(["customer_id", "product_id"]);
    makearray.push({
