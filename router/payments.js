...
router.post("/createpayment", authenticate, async (req, res) => {
  ...
  try {
    ...
    /* ///////////////////////////// GRAM UPDATE //////////////////////////////  */
    gramsvalue = await Rate.find({})
      .limit(1)
      .sort({ rowid: "desc" })
      .populate({ path: "transation_id,user_id", strictPopulate: false })
      .exec();
    ...
    Payment.findByIdAndUpdate(pay._id, { grams: ggrams }, (err, paydetails) => {
      ...
      res
        .status(200)
        .send({ status: "true", message: "Payment Saved", data: paydetails });
    });
    /* /////////////////////////////////////////////////////////////////////////  */
  } catch (err) {
...
router.put("/:id", async (req, res) => {
  Payment.findByIdAndUpdate(req.params.id, req.body, (err, user) => {
    ...
    res
      .status(200)
      .send({ status: "true", message: "Payment Updated Success", data: user });
  });
});
...
router.delete("/:id", async (req, res) => {
  Payment.findByIdAndRemove(req.params.id, req.body, (err, user) => {
    ...
    res
      .status(200)
      .send({ status: "true", message: "Payment Deleted Success", data: user });
  });
});
...
router.get("/getpayment", async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({
      status: "true",
      message: "Payment List Loading Success",
      data: results,
    });
  } catch (err) {
    res
